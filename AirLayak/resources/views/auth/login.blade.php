...
<body class="bg-[#04342C] text-white min-h-screen flex">
        ...
        <div class="w-full max-w-md p-10 rounded-2xl shadow-2xl">
            ...
            <form method="POST" action="{{ route('login.submit') }}">
                ...
                <div class="flex bg-gray-100 p-1 rounded-lg mb-6 text-sm">
                    ...
                    <button type="button" onclick="setRole(this, 'pdam')" class="role-btn flex-1 py-2 text-gray-400">
                        PDAM
                    </button>
                    <button type="button" onclick="setRole(this, 'dinkes')" class="role-btn flex-1 py-2 text-gray-400">
                        Dinkes
                    </button>
                </div>
                ...
                <!-- Error -->
                @if ($errors->any())
                    <div class="text-red-500 text-xs mb-2">{{ $errors->first() }}</div>
                @endif

                <!-- Submit -->
                <button type="submit"
                    class="w-full h-12 bg-teal-600 text-white rounded-xl font-semibold hover:bg-teal-800 transition mt-3">
                    Masuk
                </button>

                <div class="text-right text-xs text-teal-600 mb-4 mt-2 cursor-pointer">
    ...
    <script>
        function setRole(btn, role) {
            ...
            // Toggle Google login & guest section
