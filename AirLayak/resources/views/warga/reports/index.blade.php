        ...
        <div class="grid gap-6">
            @forelse($reports as $report)
                <div class="border rounded-lg p-6 shadow-sm bg-white">
                    {{-- MODIFIKASI DI BAGIAN INI (Tambahkan Tombol Lihat Detail) --}}
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <span class="inline-block px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-800">
                                {{ $report->statusLabel() }}
                            </span>
                            <h2 class="text-xl font-semibold mt-2 capitalize">{{ str_replace('_', ' ', $report->category) }}
                            </h2>
                        ...
                        <a href="{{ route('laporan.show', $report->id) }}"
                            class="text-sm font-medium text-teal-600 hover:text-teal-800 bg-teal-50 px-3 py-1.5 rounded-lg transition-colors border border-teal-100">
                            Lihat Timeline & Detail &rarr;
                        </a>
                    </div>
                    ...
                    @if($report->status === 'extension_requested' && $report->activeExtension)
                        <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-md mb-4">
                            ...
                            <p class="text-sm text-yellow-700">Estimasi Baru:
                                {{ $report->activeExtension->proposed_eta_at->format('d M Y, H:i') }}</p>

                            <form action="{{ route('laporan.extension.respond', $report->activeExtension->id) }}" method="POST"
                                class="mt-3 flex gap-2">
                                @csrf
                                <button type="submit" name="action" value="approve"
                                    class="bg-yellow-600 text-white px-4 py-2 rounded text-sm hover:bg-yellow-700">Setujui</button>
                                <button type="submit" name="action" value="reject"
                                    class="bg-gray-200 text-gray-800 px-4 py-2 rounded text-sm hover:bg-gray-300">Tolak</button>
                            </form>
                        </div>
                    @endif
                        ...
                        @if($report->status === 'pending')
                            <form action="{{ route('laporan.cancel', $report->id) }}" method="POST">
                                ...
                                <button type="submit" class="text-red-600 hover:text-red-800 font-medium"
                                    onclick="return confirm('Yakin ingin membatalkan laporan ini?')">Batalkan Laporan</button>
                            </form>
                        @endif
                        ...
                        @if(in_array($report->status, ['awaiting_confirmation', 'in_progress']))
                            <form action="{{ route('laporan.resolve', $report->id) }}" method="POST">
                                ...
                                <button type="submit"
                                    ...
                                    onclick="return confirm('Konfirmasi bahwa masalah air sudah teratasi?')">Tandai Selesai</button>
                            </form>
                        @endif
                    </div>
                </div>
